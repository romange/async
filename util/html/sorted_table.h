...
class SortedTable {
 public:
  ...
  static void StartTable(const std::vector<absl::string_view>& header, std::string* dest);

  static void Row(const std::vector<absl::string_view>& row, std::string* dest);
...
std::string SortedTable::Row(absl::string_view style, const Container& row) {
  std::string res("<tr>");
  std::string td_tag;
  if (style.empty()) {
    td_tag = "<td>";
